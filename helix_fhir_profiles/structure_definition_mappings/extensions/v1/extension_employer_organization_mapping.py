...
profile_name = "Extension - Employer Organization"


class ExtensionEmployerOrganization:
    @staticmethod
    def get_profile_id() -> AutoMapperTextLikeBase:
        return A.text("ex-employer-organization")

    @staticmethod
    def get_profile_url() -> AutoMapperTextLikeBase:
        return A.concat(
            ProfileDefinition.get_url_base(),
            ExtensionEmployerOrganization.get_profile_id(),
        )
...
def mapping(parameters: Dict[str, Any]) -> List[AutoMapper]:
    ...
    mapper = AutoMapper(
        view=parameters["view_extension_employer_organization"],
        source_view=parameters["view_profile_config"],
        use_schema=False,
    ).complex(
        ProfileDefinition(
            profile_id=FhirId(ExtensionEmployerOrganization.get_profile_id()),
            profile_name="Extension - Employer Organization",
            computer_friendly_profile_name="ExtensionEmployerOrganization",
            fhir_resource_type="Extension",
            description="Extension to reference an employer organization",
            context=FhirList(
                [
                    StructureDefinitionContext(
